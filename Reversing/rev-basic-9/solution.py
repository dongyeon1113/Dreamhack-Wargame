
# S-Box 테이블 
s_box = [
    
]

# 암호문
comp1 = [
    0x7E, 0x7D, 0x9A, 0x8B, 0x25, 0x2D, 0xD5, 0x3D,
]
comp2 = [
    0x03, 0x2B, 0x38, 0x98, 0x27, 0x9F, 0x4F, 0xBC,
]
comp3 = [
    0x2A, 0x79, 0x00, 0x7D, 0xC4, 0x2A, 0x4F, 0x58
]

# ROL (Rotate Left) 함수 구현
# 암호화 루틴에 쓰인 ROR(Rotate Right)의 역연산
def ROL(value, bits):
    return ((value << bits) | (value >> (8 - bits))) & 0xFF

# 메인 복호화 함수
def decrypt(comp):
    key=bytearray(b'I_am_KEY') # 복호화 키
    input=list(comp)           # 데이터 복사

    # 16 라운드 역순 반복 
    for i in range(16):
        # 블록 내 8바이트 데이터 처리
        for j in range(8):
            # 복호화 공식 (Inverse Operation)
            # ROL : ROR 5비트 회전된 값을 원상복구
            # (-) s_box : 더해졌던 S-Box 값을 뺌
            # (8-j)&7 : 인덱스를 역순으로 추적하여 복호화
            temp=ROL(input[(8-j)&7], 5)-s_box[key[8-j-1]^input[((8-j-1)&7)]]
            
            # 0xFF로 마스킹하여 1바이트 범위(0~255)로 보정 (음수 방지)
            input[(8-j)&7]=temp&0xFF
    return input

# 각 암호문 블록 복호화 수행
result=decrypt(comp1)
result.extend(decrypt(comp2))
result.extend(decrypt(comp3))

# 복구된 Flag 문자열 출력
for i in range(24):
    print(chr(result[i]), end='')
