...
## 5. Thoughts
난이도가 한 단계 올라가면서, 이전 문제들과 유형은 비슷하지만 분석해야 할 깊이가 달라졌음을 체감했다. 특히 이번 문제 해결의 걸림돌은 역연산 알고리즘 구현이 아니라, **"C언어 문자열의 끝은 항상 NULL(\0)로 끝난다"**는 아주 기초적인 규칙을 간과한 점이었다. 복잡한 어셈블리 명령어에 너무 집중하느라 오히려 기본적인 메모리 구조를 놓친 것이다.
이번 문제를 통해 **"복잡한 문제일수록 해답은 가장 기본적인 원리에 있다"**는 교훈을 얻었다. 앞으로는 데이터가 메모리에 어떻게 저장되는지 기본부터 확실히 짚고 넘어가는 습관을 들여야겠다.
