...
## 4. Thoughts
항상 입력값을 받아서 연산을 하고 준비되어있는 data와 비교하는 문제를 풀다가 
이번에 오랜만에 패치하는 문제를 풀었는데 nanosleep을 패치했다고 문제가 끝나는게 아니어서 당황했다.
여기저기 값을 수정하고 이것저것 만지느라 삽질을 많이해서 문제를 해결하는데 오래걸렸다.
하지만 내가 디버깅을 못하도록 출제자가 antidebug장치(integrity_check)를 걸어놓을 수도 있다는것을 깨달아서 의미있었다. 
